...
for animal in animal_two_numpy_file:
    ...
    slope_intercept_df = pd.DataFrame(data = {'animal number': [animal]*len(slope_epochs), 'Slope': slope_epochs, 'Intercept':intercept_epochs})
    results = pd.DataFrame(data = {animal:list_mean})
    ETX_2_numpyfiles.append(results)
    ETX_slope_intercept_2.append(slope_intercept_df)

merged_2_numpyfiles = pd.concat(ETX_2_numpyfiles, axis = 1)
merged_2_slope_intercepts = pd.concat(ETX_slope_intercept_2, axis=1)
os.chdir('/home/melissa/Results')
merged_2_numpyfiles.to_csv('wake_power_ETX_2.csv', index=True)
merged_2_slope_intercepts.to_csv('wake_slope_intercepts_ETX.csv', index=True)
